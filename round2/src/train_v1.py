...
def state_dim_for(top_k: int = DEFAULT_TOP_K) -> int:
    return MARIO_FEATURE_DIM + top_k * BLURP_FEATURE_DIM
...
def preprocess_observation(observation: dict[str, Any], top_k: int = DEFAULT_TOP_K) -> np.ndarray:
    state, _risk_info = _extract_features_and_risk(observation, top_k)
    return state
...
def shaped_reward(
    ...
    risk_now = estimate_risk(observation, top_k)
    risk_next = estimate_risk(next_observation, top_k)
    mario = _safe_float_array(observation.get("mario", np.zeros(5, dtype=np.float32)), (5,))
    m_left, _m_top, m_right, _m_bottom, m_vx = [float(x) for x in mario]
    ...
    shaped -= 0.30 * risk_next.risk
    shaped += 0.15 * (risk_now.risk - risk_next.risk)

    if risk_now.risk >= RISK_THRESHOLD:
        if action == risk_now.avoid_action:
            shaped += 0.05
        if risk_now.avoid_vx_sign != 0.0 and (m_vx * risk_now.avoid_vx_sign) > 0.0:
            shaped += 0.04

    wall_distance = min(m_left, WORLD_WIDTH - m_right)
    if wall_distance / WORLD_WIDTH < 0.06:
        shaped -= 0.03

    speed_ratio = abs(m_vx) / MAX_MARIO_SPEED
    if speed_ratio > 0.85:
        shaped -= 0.02 * min(speed_ratio - 0.85, 1.0)

    return float(np.clip(shaped, -10.0, 100.0))
...
class TrainConfig:
    ...
    top_k: int = DEFAULT_TOP_K
    seed: int = 42
    save_path: str = "avoid_blurp_dqn.pt"
...
def evaluate_once(path: str = "avoid_blurp_dqn.pt") -> float:
    if gym is None:
        raise RuntimeError("gymnasium is not installed.")
    import kymnasium as _kymnasium  # noqa: F401 - registers the environment.
