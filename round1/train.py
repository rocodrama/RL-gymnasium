...
def train(config: TrainConfig = TrainConfig()):
    """제출용 학습 루프(실제 Gym 환경 step 사용, FastEnv 미사용)."""
    def parse_monster_map(tiles: np.ndarray) -> Dict[Tuple[int, int], Tuple[int, int]]:
        """현재 프레임에서 몬스터 좌표/속성을 사전으로 구성한다."""
        monster_map: Dict[Tuple[int, int], Tuple[int, int]] = {}
        for r in tiles:
            x, y, obj, attr = map(int, r)
            if obj in MONSTER_OBJECT_IDS:
                monster_map[(x, y)] = (obj, attr)
        return monster_map

    def parse_sword_map(tiles: np.ndarray, dropped_positions: set[Tuple[int, int]]) -> Dict[Tuple[int, int], int]:
        """현재 프레임에서 드롭 검을 제외한 검 좌표/속성 사전을 구성한다."""
        sword_map: Dict[Tuple[int, int], int] = {}
        for r in tiles:
            x, y, obj, attr = map(int, r)
            if obj == OBJ_SWORD and (x, y) not in dropped_positions:
                sword_map[(x, y)] = attr
        return sword_map

    def nearest_unseen_sword_distance(
        x: int,
        y: int,
        sword_map: Dict[Tuple[int, int], int],
        collected_colors: set[int],
    ) -> int:
        """미수집 색 검까지의 맨해튼 최단거리를 반환한다."""
        best = 10**9
        for (sx, sy), sword_color in sword_map.items():
            if sword_color in collected_colors:
                continue
            d = abs(sx - x) + abs(sy - y)
            if d < best:
                best = d
    ...
    def build_obs_block_grid(static_grid: np.ndarray, tiles: np.ndarray, sword_color: int) -> np.ndarray:
        """관측 기준 장애물 맵을 구성한다(동일 색 몬스터는 통과 가능 처리)."""
        grid = static_grid.copy()
        for r in tiles:
            x, y, obj, attr = map(int, r)
            if obj not in MONSTER_OBJECT_IDS:
                continue
            if sword_color != 0 and attr == sword_color:
                continue
            grid[x, y] = WALL
        return grid
    ...
    first_tiles = np.asarray(first_obs["tiles"])
    static_grid, goal_pos, cloud_positions, initial_sword_map, initial_monster_map = build_static_layout_from_tiles(first_tiles)
    start_x, start_y, _, _ = map(int, first_obs["link"])
    target_sword_color_num = len(set(initial_sword_map.values()))

    start_dist_map = build_distance_map(build_obs_block_grid(static_grid, first_tiles, 0), goal_pos)
    start_dist = int(start_dist_map[start_x, start_y])

    print("========== SUBMIT TRAIN CHECK ==========")
    print("start:", (start_x, start_y))
    print("goal :", goal_pos)
    print("distance with alive monsters blocked:", start_dist)
    print("cloud count:", len(cloud_positions))
    print("sword count:", len(initial_sword_map))
    print("monster count:", len(initial_monster_map))
    print("========================================")
    ...
    for episode in range(1, config.episodes + 1):
        obs, info = env.reset()
        runtime = FinalEvalAgent()
        runtime.sync_episode_memory(obs, info)
        state = runtime.extract_state(obs)
        ...
        while steps < config.max_steps and not done:
            old_tiles = np.asarray(obs["tiles"])
            old_link = tuple(map(int, obs["link"]))
            old_x, old_y, old_sword, old_dir = old_link
            old_pos = (old_x, old_y)
            old_collected_colors = set(runtime.collected_sword_colors)
            old_monster_map = parse_monster_map(old_tiles)
            old_sword_map = parse_sword_map(old_tiles, runtime.dropped_sword_positions)

            old_dist_map = build_distance_map(build_obs_block_grid(static_grid, old_tiles, old_sword), goal_pos)
            old_dist = int(old_dist_map[old_x, old_y])
            old_unseen_dist = nearest_unseen_sword_distance(old_x, old_y, old_sword_map, old_collected_colors)
            ...
            if episode <= config.random_episodes:
                ...
                missing_colors = target_sword_color_num - state[10]
            ...
            runtime.sync_episode_memory(next_obs, next_info)
            next_state = runtime.extract_state(next_obs)
            agent.ensure_state(next_state)
            next_tiles = np.asarray(next_obs["tiles"])
            new_x, new_y, new_sword, _ = map(int, next_obs["link"])
            new_pos = (new_x, new_y)
            moved = new_pos != old_pos
            next_monster_map = parse_monster_map(next_tiles)
            next_sword_map = parse_sword_map(next_tiles, runtime.dropped_sword_positions)
            next_sword_color_num = next_state[10]

            new_dist_map = build_distance_map(build_obs_block_grid(static_grid, next_tiles, new_sword), goal_pos)
            new_dist = int(new_dist_map[new_x, new_y])
            new_unseen_dist = nearest_unseen_sword_distance(new_x, new_y, next_sword_map, runtime.collected_sword_colors)
            ...
            if moved and old_dist >= 0 and new_dist >= 0:
                ...
                if (
                    active_focus_colors >= target_sword_color_num
                    and sword_color_num < target_sword_color_num
            ...
            if action_idx == IDX_ATTACK:
                ...
                if state[3] == 0:
                    ...
                    monster_obj = front_type_to_monster_obj(front_type)
                    if required_hits(monster_obj, front_attr, state[3]) >= INF_HITS:
                        reward -= 0.30
            ...
            if action_idx == IDX_PICKUP:
                ...
                if old_sword == 0 and new_sword != 0:
                    env_bonus += 5.0
                elif old_sword != 0 and new_sword != 0 and old_sword != new_sword:
                    can_force_swap_new_color = (current_sword_used == 0) and is_new_color
                    if current_sword_used == 0 and not can_force_swap_new_color:
                        env_bonus -= SWAP_BEFORE_USE_PENALTY
                    else:
                        env_bonus += 2.0
                        if can_force_swap_new_color:
                            env_bonus -= SWAP_BEFORE_USE_PENALTY * 0.5
                if is_new_color:
                    env_bonus += NEW_COLOR_PICKUP_BONUS

            if action_idx == IDX_DROP and drop_swap_ready and old_sword != 0 and new_sword == 0:
                env_bonus += 0.5

            if action_idx == IDX_ATTACK:
                if old_sword == 0:
                    env_bonus -= BARE_HAND_ATTACK_PENALTY
                else:
                    dx, dy = forward_delta(old_dir)
                    target = (old_x + dx, old_y + dy)
                    old_front_monster = old_monster_map.get(target)
                    if old_front_monster is not None:
                        monster_obj, monster_color = old_front_monster
                        if required_hits(monster_obj, monster_color, old_sword) >= INF_HITS:
                            env_bonus -= 0.35
                        else:
                            if current_sword_used == 0:
                                env_bonus += FIRST_USE_HIT_REWARD
                            if target not in next_monster_map:
                                env_bonus += get_attack_reward(monster_obj, monster_color, old_sword)
                    else:
                        env_bonus -= 0.35
            ...
            done = terminated or truncated or (steps + 1 >= config.max_steps)
            ...
            if terminated:
                ...
                missing_to_target = max(0, target_sword_color_num - next_sword_color_num)
                if missing_to_target > 0 and active_focus_colors >= target_sword_color_num:
                    reward -= config.non_full_goal_extra_penalty * missing_to_target
                if next_sword_color_num >= target_sword_color_num:
