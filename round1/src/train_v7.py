...
UNSEEN_SWORD_PROGRESS_GAIN = 0.90
UNSEEN_SWORD_RETREAT_PENALTY = 0.60
...
class TrainConfig:
    ...
    explore_boost_epsilon: float = 0.26
    explore_boost_after_steps: int = 55
    min_colors_for_goal_focus: int = 5
    early_goal_progress_gain: float = 0.90
    goal_reward_base: float = 550.0
    goal_reward_per_color: float = 45.0
    goal_missing_color_penalty: float = 70.0
    model_path: str = "round7_adaptive_colors.pkl"
...
def get_candidate_indices(state):
    ...
    if current_tile_sword != 0 and sword != current_tile_sword and (sword == 0 or current_sword_used == 1):
        return [IDX_PICKUP]
...
class FastZeldaMonsterEnv:
    ...
    def build_current_block_grid(self) -> np.ndarray:
        ...
        for (mx, my), (_, monster_color) in self.monster_map.items():
            # For path-distance shaping, monsters with the same color as the current sword
            # are treated as passable tiles.
...
class Round7EvalAgent(kym.Agent):
    ...
    for episode in range(1, config.episodes + 1):
        ...
        while steps < config.max_steps and not done:
            ...
            if moved and old_dist >= 0 and new_dist >= 0:
                ...
                if sword_color_num < config.min_colors_for_goal_focus:
            ...
            if state_visit_count[next_state] >= 22:
                truncated = True

            done = terminated or truncated

            if terminated:
                missing_for_focus = max(0, config.min_colors_for_goal_focus - next_sword_color_num)
                reward += (
                    config.goal_reward_base
                    + config.goal_reward_per_color * next_sword_color_num
                    - config.goal_missing_color_penalty * missing_for_focus
                )
                success_count += 1
...
def run(model_path: str = "round7_adaptive_colors.pkl"):
    agent = Round7EvalAgent.load(model_path)
    kym.evaluate(
