...
class TrainConfig:
    ...
    model_path: str = "round5_fast_drop.pkl"
...
def get_candidate_indices(state):
    ...
    front_attr = state[7]
    sword = state[3]
    current_tile_sword = state[11]

    candidates = [IDX_STOP, IDX_LEFT, IDX_RIGHT]
...
class FastZeldaMonsterEnv:
    ...
    def nearest_sword_pos(self) -> Tuple[int, int] | None:
        if not self.sword_map:
            return None

        best_pos = None
        best_dist = 10**9
        for (sx, sy), scolor in self.sword_map.items():
            if self.sword != 0 and scolor == self.sword:
                continue
    ...
    def get_state(self):
        ...
        nearest_sword = self.nearest_sword_pos()
        sword_fb, sword_lr = compute_relative_features(self.x, self.y, self.direction, nearest_sword)
        ...
        if action == ACTION_STOP:
            ...
            if current_sword is not None:
                if self.sword == 0:
                    ...
                    self.sword = current_sword
                    self.sword_map[(self.x, self.y)] = old_sword
                    reward_bonus += 2.0
            ...
            # Disable bare-hand attack.
            if self.sword != 0 and front_type in (FRONT_TURTLENACK, FRONT_KEESE, FRONT_MOBLIN, FRONT_ARMOS):
                monster_obj, monster_color = self.monster_map[(nx, ny)]
...
class Round5EvalAgent(kym.Agent):
    ...
    def nearest_sword_pos_from_obs(self, x: int, y: int, sword: int, tiles: np.ndarray):
        best_pos = None
        best_dist = 10**9
        for r in tiles:
            tx, ty, obj, attr = map(int, r)
            if obj == OBJ_SWORD:
                if sword != 0 and attr == sword:
                    continue
                d = abs(tx - x) + abs(ty - y)
                if d < best_dist:
                    best_dist = d
    ...
    def extract_state(self, observation):
        ...
        nearest_sword = self.nearest_sword_pos_from_obs(x, y, sword, tiles)
        sword_fb, sword_lr = compute_relative_features(x, y, direction, nearest_sword)
    ...
    for episode in range(1, config.episodes + 1):
        ...
        while steps < config.max_steps and not done:
            ...
            if old_dist >= 0 and new_dist >= 0:
                reward += 1.2 * (old_dist - new_dist)

            if old_dist >= 0 and new_dist > old_dist:
                reward -= 0.10

            if front_type == FRONT_CLOUD_BLOCKED and action_idx == IDX_STOP:
                reward += 0.03
            ...
            if state[3] == 0 and current_has_sword == 0:
                if sword_fb == 1 and action_idx == IDX_FORWARD:
                    reward += 0.05

                if sword_lr == 1 and action_idx == IDX_RIGHT:
                    reward += 0.03
                elif sword_lr == -1 and action_idx == IDX_LEFT:
                    reward += 0.03
...
def run(model_path: str = "round5_fast_drop.pkl"):
