...
# 2 -> right
# 3 -> forward
ACTIONS = [ACTION_STOP, ACTION_TURN_LEFT, ACTION_TURN_RIGHT, ACTION_FORWARD]
...
class TrainConfig:
    ...
    model_path: str = "round3_cloud_wait.pkl"
...
def build_state(
    ...
    return (x, y, direction, goal_fb, goal_lr, front_type)
...
def get_candidate_indices(front_type: int):
    if front_type == FRONT_WALL:
        return [1, 2]          # left, right
    if front_type == FRONT_CLOUD_BLOCKED:
        return [0, 1, 2]       # stop, left, right
    return [1, 2, 3]           # left, right, forward
...
class FastZeldaCloudEnv:
    ...
    def compute_front_type(self) -> int:
        ...
        if self.is_cloud_at(nx, ny):
            ...
            if is_cloud_blocked(attr):
                return FRONT_CLOUD_BLOCKED

        return FRONT_OPEN

    def get_state(self):
        front_type = self.compute_front_type()
        return build_state(
            x=self.x,
            y=self.y,
...
class Round3EvalAgent(kym.Agent):
    ...
    def act(self, observation, info):
        ...
        front_type = state[5]
        candidate_indices = get_candidate_indices(front_type)
...
def train(config: TrainConfig = TrainConfig()):
    ...
    for episode in range(1, config.episodes + 1):
        ...
        while steps < config.max_steps and not done:
            ...
            # waiting in front of blocked cloud is slightly better than useless spinning
...
def run(model_path: str = "round3_cloud_wait.pkl"):
    agent = Round3EvalAgent.load(model_path)
    kym.evaluate(
